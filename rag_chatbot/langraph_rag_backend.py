...
# Compatibility patch for langgraph-checkpoint >= 3.0.0 with SqliteSaver
...
def ingest_pdf(file_bytes: bytes, thread_id: str, filename: Optional[str] = None) -> dict:
    ...
    try:
        ...
        if not docs or not any(doc.page_content.strip() for doc in docs):
            raise ValueError(
                "Could not extract any text from the PDF. It may be an image-only scanned document or empty."
            )
        ...
        if not chunks:
            raise ValueError("No valid text chunks could be generated from the document.")
